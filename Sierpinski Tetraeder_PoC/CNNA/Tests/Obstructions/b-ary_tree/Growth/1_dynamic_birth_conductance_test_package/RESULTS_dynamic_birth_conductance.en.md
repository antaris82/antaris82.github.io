...
- a newborn cell has no UV-tail of its own;
- the newborn immediately acts as a UV-tail/backreaction for its parent line;
...
- geometric lengths are not conductances.
...
Each mode has `3280` nodes and `3279` birth events at the final level.
...
Thus, the conductance system is not static after a sibling triple is completed.
...
### 1. Sequential birth generally violates sibling equality
...
Thus, `g1 = g2 = g3` is not general under sequential birth.
...
decreases in `log`, and remains small but nonzero in `saturating`.

This shows that imbalance depends on the update rule, but is not an artifact of a static `[1,2,1]` input.
...
The log-ratio remains positive in all modes up to level 7:
...
Thus, the directed influence/backreaction system is not forward/reverse balanced.
...
This means that many support cycles exist in the influence graph.  
...
The next diagnostic should address two questions:

1. Are the support cycles merely graph artifacts resulting from directed response tracking?
