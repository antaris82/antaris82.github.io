...
- `residual pre/post` measures the actual pair-edge interfan residual before and after Z3 phase propagation.
- `graph edges` and `conflict frac` measure whether the propagated labels are compatible across the chosen graph.
- `pair harm` is the H2 projection of the actual pair-transport field after propagation.
