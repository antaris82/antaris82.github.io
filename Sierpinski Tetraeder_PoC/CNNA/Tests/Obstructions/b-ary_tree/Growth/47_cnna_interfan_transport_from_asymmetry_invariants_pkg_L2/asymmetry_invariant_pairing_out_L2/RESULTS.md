...
This script implements that test. The asymmetry score does **not** use `delta_beta2` for ranking, so the test asks whether the provenance/response data select the handle before looking at the topological result.
...
The uncomfortable part is equally important:
...
That means the older generated `symmetrized_birth` control is not strict enough for this specific question, and `no_backreaction` is not a no-asymmetry control. Sequential older-sibling environment already creates a directed imprint. Therefore the current evidence supports:
...
The handle is still an offered candidate that is then applied. The next test must integrate the rule into the growth loop:
...
Then compare real growth against strict symmetrized/no-sibling/no-backreaction controls before any after-the-fact candidate application.
