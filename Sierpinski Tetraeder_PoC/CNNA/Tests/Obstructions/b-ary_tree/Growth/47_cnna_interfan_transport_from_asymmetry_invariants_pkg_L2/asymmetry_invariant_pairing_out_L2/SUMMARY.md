...
This package implements the sharpened test requested after Claude's critique:
...
- `no_backreaction` also does not collapse, because older-sibling birth-environment feed-forward alone already creates a directed asymmetry. This means the active signal is not purely backreaction; it is sequential birth-environment plus optional backreaction.

The result supports a selective complement-pairing hypothesis, but it still does not prove that the growth rule must execute the offered handle. The next step must make gated non-shelling pairing an in-growth event and test whether controls fail before applying the move.
