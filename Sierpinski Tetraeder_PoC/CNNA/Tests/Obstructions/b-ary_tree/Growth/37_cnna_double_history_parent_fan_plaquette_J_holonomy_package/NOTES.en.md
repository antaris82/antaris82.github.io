...
The primary L5/L6 runs exhibit a persistent directed-cyclic `Jfixed ≈ 2/3` signature, but no nontrivial Z2 sign-lift obstruction (`z2obs = 0`). The same-label quotient glues much more consistently with fixed signs than the cyclic root-shift. Therefore, this package supports the presence of a local Z3 label/gluing signature, but does not yet prove Stufe-4 locking.
