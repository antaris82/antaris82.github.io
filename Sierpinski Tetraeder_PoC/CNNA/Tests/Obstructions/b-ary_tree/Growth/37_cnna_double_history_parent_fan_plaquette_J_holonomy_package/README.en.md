...
The test determines whether a local Z3/parent-fan J sector remains nontrivial under this provenance identification.
...
- `diagonal`, `trace_scalar`, `common_mean_diagonal`: must eliminate the noncommutative DtN carrier.
- `symmetrized_birth`: should suppress the K/J amplitude.
- `random_same_level_cycle_baseline`: checks whether the quotient signal is structured.

## Reading rule

A genuine Level-4 sign/holonomy candidate would require more than local J-validity. It should produce a double-history-specific sign/holonomy obstruction that vanishes for identical histories, collapses under symmetrized birth and commuting reductions, and is not reproduced by random same-level cycles.
