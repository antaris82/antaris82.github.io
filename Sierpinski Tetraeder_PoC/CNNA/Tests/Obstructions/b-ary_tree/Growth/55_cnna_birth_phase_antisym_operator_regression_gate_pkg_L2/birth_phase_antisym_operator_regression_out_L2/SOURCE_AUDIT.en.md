...
Because the antisymmetric birth-transport branch changes the total axial field but still projects to an approximately zero harmonic axial ratio, the next test should not yet assume operator closure.  The next falsification/localization gate is:

```text
test_directed_transport_harmonic_obstruction_localization_gate.py
```

Goal: Determine whether the harmonic cancellation is caused by (1) the commutator definition `skew([A_ab,A_bc])`, (2) the H² carrier/cap geometry, (3) face-normal/birth-normal projection, or (4) global exact/coboundary cancellation of a locally directed transport field.
