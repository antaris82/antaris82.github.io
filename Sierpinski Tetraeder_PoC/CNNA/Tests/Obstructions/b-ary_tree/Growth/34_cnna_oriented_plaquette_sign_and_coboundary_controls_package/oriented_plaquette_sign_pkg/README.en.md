...
This package analyzes the first simplicial DtN plaquette-frustration signal.

It retains the same event-resolved ternary growth and dynamic Record/Live DtN data, but now tests whether the face signal behaves as an oriented noncommutative plaquette effect:

1. Reversing `(a,b,c)` to `(a,c,b)` must flip the commutator/skew sign;
2. first-order edge circulation must telescope;
3. diagonal, trace-scalar, and common-basis commuting ablations must eliminate the signal;
