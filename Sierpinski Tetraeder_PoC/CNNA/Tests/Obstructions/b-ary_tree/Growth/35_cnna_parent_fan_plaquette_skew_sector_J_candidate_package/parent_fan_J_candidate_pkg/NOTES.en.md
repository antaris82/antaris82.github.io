...
1. parent-fan/full/live carrier is much stronger than sibling/random/symmetrized controls;
2. orientation reversal sends `K` and `J` to their negatives;
3. diagonal/trace/common-commuting reductions eliminate the signal;
...
The real-growth parent-fan live/full carrier yields a stable local rank-2 skew
sector with `J2_plane_residual` near numerical zero and a clean orientation flip.
Diagonal/scalar/common-commuting controls collapse.

This is evidence for a local face-level J-candidate carrier, but not yet for a
