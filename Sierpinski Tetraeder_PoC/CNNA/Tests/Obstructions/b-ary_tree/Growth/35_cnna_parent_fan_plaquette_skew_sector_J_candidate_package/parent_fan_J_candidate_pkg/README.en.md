...
This package is the first deliberately J-near test following the simplicial DtN
...
The candidate is not freely chosen.  It is extracted from
...
normalizes `J = K / omega` on that plane.  The nontrivial aspect is therefore not
the linear algebra fact that a nonzero 3x3 skew matrix defines a plane rotation,
but whether the robust carrier survives controls and exhibits correct orientation
behavior.
...
Even if this is true, it is still only a local skew-sector J-candidate.
