# SOURCE AUDIT — operator composition gate
...
The forum point is explicitly respected here: stating that a sign is conventional is valid only if the entire associated structure package is transported coherently.  One must not replace `i` with `-i` while leaving branch cuts, argument conventions, upper/lower half-plane, Fourier signs, causal signs, positivity, and `*` unchanged.
...
`A^# = R A^T R` is a candidate real anti-involution on matrices with finite coefficients.  It is not yet a CNNA-derived `*` because:
...
The only legitimate claim is whether the generated family of real operators is compatible or incompatible with this candidate reversal.
