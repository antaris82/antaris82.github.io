...
Canonical dimensions lie in the trivial $S_b$ component.
...
Event structures are of interest as subsequent target or comparative structures. They could describe how emergent events, conflicts, and a causal order arise from a CNNA-derived pre-structure.
...
Any substrate that already contains a directed temporal, causal, orientation, or phase structure bypasses the actual CNNA test. In that case, the missing second axis would not be derived, but imported.
...
This version lists only artifacts that have either been appended or are explicitly referenced as Hugging Face visualizations. Older package names, non-attached post-tests, and hypothetical future implementations are no longer listed as a reproducible artifact basis for this file.
